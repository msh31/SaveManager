...
2. `Translations translations; translations.init()`: loads the Steam appid/name and Ubisoft id/name lookup tables. Detectors use these to turn opaque folder ids into game names.
3. `Blacklist blacklist; blacklist.init()`: loads the user-editable blacklist set (default seeds: *The Crew Motorfest*, *Skull and Bones*).
4. `CWindowManager window;`: the constructor runs `setup_opengl()` (GLFW, OpenGL 3.3 core, GLAD) and `setup_imgui()` (context, fonts, backends). Nothing is drawn yet.
5. `CApp app( blacklist, translations );`: borrows both as references. Constructing `CApp` constructs its `CConfig m_config` member, whose constructor creates the config/log/backup/plugin/cache dirs and loads `config.json` from disk.
6. `app.init()`: applies the theme, registers the views (Dashboard, Editor, Transfer, About, Log, then Settings) into the UI manager, and constructs the background `CShader`.
...
8. Exit on `q` or window close, unwinding in reverse: `CApp` dtor (`m_task_runner.shutdown()`), `CConfig` dtor (saves `config.json`), `CWindowManager` dtor (ImGui and GLFW teardown).
...
- `main` (stack) owns: `Translations`, `Blacklist`, `CWindowManager`, `CApp`.
- `CApp` owns: `CConfig m_config`, `CUIManager m_ui_manager`, `CTaskRunner m_task_runner`, `std::optional<CShader>`. Borrows: `Blacklist&`, `const Translations&`.
...
- `CDashboardView` owns: `CBackupsView`, its own `CTaskRunner`, the game cache (`m_game_cache`), the detection/backup `std::future`s. Borrows: `CConfig&`, `const Blacklist&`, `const Translations&`.

Detection is not owned by `CApp`. Each view that needs saves runs it: `CDashboardView::on_enter()` starts `Detection::find_saves()` on a background thread and holds the result in `m_result`. There is no central detection result and no scheduler in the GUI. (The pre-1.8.0 doc claimed otherwise.)

> **Why does each view re-run detection instead of `CApp` running it once?** Not intentional, a known issue targeted for a 1.8.0 fix. `CDashboardView` (`on_enter` plus Refresh) and `CTransferView` both call `Detection::find_saves()` independently, so detection runs more than once per session and the two views can hold divergent snapshots. Intended fix: hoist detection to a single owner (`CApp` or a small detection service) and hand views a borrowed result. Current dev-branch debt, not a rewrite concern (1.8.0 is unreleased, `main` is still 1.7.1).
...
1. `CDashboardView::on_enter()` (or the Refresh button) starts a `std::async` running `Detection::find_saves(blacklist, translations)` on a worker thread, storing the result in `m_result` under `m_result_mutex`.
...
4. Back on the UI thread, `render()` polls the future. When ready it copies into `m_games_snapshot`; when the game count changes it calls `on_result_changed()`, which rebuilds the per-game cache (save files, backups, labels, conflict flags) on `m_task_runner` and swaps it in when done.
