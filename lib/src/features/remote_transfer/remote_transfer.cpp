...
bool RemoteTransfer::connect( const std::string &dest_addr, const CConfig &config, bool auth_pw,
    ...
    if ( auth_pw ) {
        ...
        if ( libssh2_userauth_publickey_fromfile( session, config.sftp.username.c_str( ),
...
void RemoteTransfer::upload_file( const fs::path &backup_path, const std::string &remote_path, const CConfig &config ) {
    ...
    sftp_handle = libssh2_sftp_open(
        sftp_session, remote_file.c_str( ), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT | LIBSSH2_FXF_TRUNC,
