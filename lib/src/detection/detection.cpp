...
void Detection::find_saves( CConfig& config, std::vector<Game>& games ) {
    ...
    if ( fs::exists( lutris_dir ) ) {
        lutris_future = std::async( std::launch::async, [lutris_dir, config, detectors]( ) -> std::vector<Game> {
    ...
    if ( fs::exists( heroic_dir ) ) {
        heroic_future = std::async( std::launch::async, [heroic_dir, config, detectors]( ) -> std::vector<Game> {
