...
## Supported Games
SaveManager makes use of a manifest based on the [ludusavi-manifest](https://github.com/mtkennerly/ludusavi-manifest) which you can find [here](https://github.com/msh31/savemanager-manifest)

Internally there are a few detection mechanisms specialized for ``Ubisoft``, ``Rockstar Games``, ``Unreal Engine4/5 Games`` & ``Minecraft`` which **are** prioritized over the manifest

> [!NOTE] 
> Minecraft: the following launchers are supported: Official, Modrinth, Curseforge, PrismLauncher (MultiMC is exclusive to linux)
