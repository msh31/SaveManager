...
void App::render_loading_screen( ) {
    ImVec2 center( ( ImGui::GetWindowSize( ).x - 24 ) * 0.5f, ( ImGui::GetWindowSize( ).y - 40 ) * 0.5f );
    ...
    ImGui::SetCursorPos(
        ImVec2( ( ImGui::GetWindowSize( ).x - ImGui::CalcTextSize( "Loading" ).x ) * 0.5f, center.y + 35 ) );
    ImGui::Text( "Loading" );
}
...
void App::render( ) {
    ...
    if ( !initialized && are_we_ready.valid( ) &&
         are_we_ready.wait_for( std::chrono::seconds( 0 ) ) == std::future_status::ready ) {
        are_we_ready.get( );
        initialized = true;
        ...
        detection_future =
            std::async( std::launch::async, [this]( ) { return Detection::find_saves( config, d_result ); } );
    }
