...
void TransferTab::render( const Fonts &fonts, Detection::DetectionResult &result, Config &config, TabState &state ) {
    ...
    bool is_connecting =
        connect_future.valid( ) && connect_future.wait_for( std::chrono::seconds( 0 ) ) != std::future_status::ready;
    ...
    if ( connect_future.valid( ) &&
         connect_future.wait_for( std::chrono::seconds( 0 ) ) == std::future_status::ready ) {
    ...
    if ( !is_connecting && !connected ) {
        if ( ImGui::Button( "Connect" ) ) {
            connect_future = std::async(
                std::launch::async,
                [this, r = remote.get( ), &config, auth = use_password_auth, pass = key_passphrase]( ) -> bool {
    ...
    ImGui::ProgressBar( file_progress, ImVec2( 300.0f, 0.0f ),
    ...
    ImGui::SameLine( 140.0f );
    ImGui::PushStyleColor( ImGuiCol_PlotHistogram, ImVec4( 0.26f, 0.59f, 0.98f, 1.0f ) );
    ImGui::ProgressBar( overall_progress, ImVec2( 300.0f, 0.0f ),
    ...
    for ( bool b : state.selected_backups )
        ...
        if ( ImGui::BeginCombo( "##game", game_names[state.selected_game_idx].c_str( ) ) ) {
            enumerate( game_names, [&]( int gi, auto &name ) {
                ...
                if ( ImGui::Selectable( name.c_str( ), is_selected ) ) {
                    ...
                    state.backups =
                        Features::get_backups( result.games[groups[static_cast<int>( gi )][0]].game_name, config );
        ...
        if ( !state.backups.empty( ) ) {
            if ( ImGui::BeginListBox( "##backups", ImVec2( -FLT_MIN, content_height ) ) ) {
                enumerate( state.backups, [&]( int gi, auto &name ) {
                    ...
                    if ( ImGui::Selectable( label.c_str( ), state.selected_backups[static_cast<int>( gi )],
                                            ImGuiSelectableFlags_AllowDoubleClick ) ) {
                        state.selected_backups[static_cast<int>( gi )] =
                            !state.selected_backups[static_cast<int>( gi )];
                    }
                } );
                ImGui::EndListBox( );
    ...
    if ( !connected ) {
        ...
        if ( ImGui::Button( "Download" ) ) {
            std::string path = current_remote_path + ( current_remote_path.back( ) == '/' ? "" : "/" ) +
                               remote_entries[selected_remote_idx].name;
        ...
        if ( ImGui::BeginListBox( "##remote_entries", ImVec2( -FLT_MIN, remote_content_height ) ) ) {
            ...
            enumerate( remote_entries, [&]( int gi, auto &entry ) {
                ...
                if ( ImGui::Selectable( label.c_str( ), selected_remote_idx == static_cast<int>( gi ),
                                        ImGuiSelectableFlags_AllowDoubleClick ) ) {
                    if ( ImGui::IsMouseDoubleClicked( 0 ) && entry.is_directory ) {
                        current_remote_path =
                            current_remote_path + ( current_remote_path.back( ) == '/' ? "" : "/" ) + entry.name;
                        remote_entries = remote->list_directory( current_remote_path );
                        selected_remote_idx = -1;
                    } else {
